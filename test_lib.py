...
def test_max_singular_value():
    ...
    bar = aqua_blue_benchmark.bar.Bar(matrix=rotation_matrix)
...
def test_matrix_multiplication():
    ...
    foo = aqua_blue_benchmark.foo.Foo(first_matrix=a, second_matrix=b)
...
def test_raises_shape_mismatch_error():
    ...
    with pytest.raises(aqua_blue_benchmark.foo.ShapeMismatchError):

        _ = aqua_blue_benchmark.foo.Foo(first_matrix=a, second_matrix=b)
